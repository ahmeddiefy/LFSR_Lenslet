...
def residual_block(x, name):
    ...
    x = tf.nn.relu(x)
    x = conv2d(x, f_in=f, f_out=f, k=kernel_size, name="conv2d-2" + "_" + name)
    x = tf.add(x , skip_conn)
    return x
    
def residual_group(x, name):
    x = conv2d(x, f_in=f, f_out=f, k=kernel_size, name="conv2d-1_0" + "_" + name)
    skip_conn_0 = x
    ...
    x = conv2d(x, f_in=f, f_out=f, k=kernel_size, name="rg-conv-0" + name)
    return tf.add(x , skip_conn_0 ) 
